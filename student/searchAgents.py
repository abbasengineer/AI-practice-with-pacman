...
class CornersProblem(SearchProblem):
    ...
    def startingState(self):
        # state represents the position and corners visited so far.
        emptySet = set()
        return (self.startingPosition, emptySet)
...
def cornersHeuristic(state, problem):
    ...
    while len(unvisitedCorners) != 0:
        (dist, cornerVisited) = min([(distance.manhattan(currentPosition, corner), corner)
...
def foodHeuristic(state, problem):
    ...
    # choose the max distance to any food
    position, foodGrid = state
    maxDist = 0
    for food in foodGrid.asList():
        distance = len(search.breadthFirstSearch(PositionSearchProblem(
                       problem.startingGameState, lambda x: 1, food, position)))
        if distance > maxDist:
            maxDist = distance
